...
$sourceFiles = @('Models.cs','RegionManagement.cs','LauncherRegion.cs','LaunchEngine.cs','WindowsPlatform.cs','Program.cs','LauncherWindow.cs','UpdateService.cs','UpdateInstaller.cs','LauncherUpdates.cs','AssemblyInfo.cs') | ForEach-Object { Join-Path $PSScriptRoot $_ }
...
$options += @(
    ...
    ('/win32icon:' + (Join-Path $PSScriptRoot 'Assets\app.ico')),
