...
请先安装好彩虹岛台服客户端。**乱码修复仅支持 Windows 11 64 位，Windows 10 暂不支持。** 兼容启动期间会临时调整系统区域，可能影响同时运行的其他非 Unicode 程序。
...
**显示游戏已启动，但文字仍乱码？** “游戏已启动”表示程序已经运行，并不代表文字已经验证。请先确认使用 Windows 11，再从“区域管理”和“启动记录”查看结果。Win10 的临时区域配置不会按预期影响新游戏进程，延长等待时间不能解决这个问题。
...
- Windows 11 **64 位**，.NET Framework **4.8 或更高版本**。如果启动时提示缺少该组件，请先安装后再打开。
- Win11 25H2 已验证区域切换与恢复，用户反馈原临时切换方案可正常运行游戏；不同 Windows 更新和游戏版本仍需验证。
- **Windows 10 / 7 不在乱码修复支持范围内。** 当前开发版本在非 Win11 系统上禁用繁体兼容启动，保留普通启动、区域检测与旧记录恢复。
...
LaTale Garden 由原生 WPF 界面、启动监控、区域管理和独立恢复进程组成。官方客户端负责游戏更新与账号登录。构建不包含游戏进程注入或文字 Hook 组件。
